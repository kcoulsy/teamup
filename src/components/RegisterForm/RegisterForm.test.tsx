...
describe('render login form component', () => {
    ...
    it('should call startRegister on submit', () => {
        ...
        wrapper.find('input[name="email"]').simulate('change', { target: { value: testUser.email } });
        wrapper.find('input[name="password"]').simulate('change', { target: { value: testUser.password } });
        wrapper.find('input[name="confirm"]').simulate('change', { target: { value: testUser.password } });
    ...
    it('should show errors if there are password problems', () => {
        ...
        wrapper.find('input[name="password"]').simulate('change', { target: { value: testUser.password } });
        wrapper.find('input[name="confirm"]').simulate('change', { target: { value: 'notthesame' } });
        wrapper.find('button[type="submit"]').simulate('click', { preventDefault: jest.fn() });

        expect(wrapper.find('li[data-error="password-mismatch"]').length).toBe(1);

        // INVALID PASSWORD (MIN LEN 3, CONTAINS NUMBER)
        wrapper.find('input[name="password"]').simulate('change', { target: { value: 'abc' } });
        wrapper.find('input[name="confirm"]').simulate('change', { target: { value: 'abc' } });
        wrapper.find('button[type="submit"]').simulate('click', { preventDefault: jest.fn() });
        ...
        wrapper.find('input[name="password"]').simulate('change', { target: { value: testUser.password } });
        wrapper.find('input[name="confirm"]').simulate('change', { target: { value: testUser.confirm } });
        wrapper.find('button[type="submit"]').simulate('click', { preventDefault: jest.fn() });
    ...
    it('should show errors if there are username problems', () => {
        ...
        wrapper.find('input[name="username"]').simulate('change', { target: { value: 'ab' } });
        wrapper.find('button[type="submit"]').simulate('click', { preventDefault: jest.fn() });
    ...
    it('should show errors if there are email problems', () => {
        ...
        // INVALID USERNAME LENGTH
        wrapper.find('input[name="email"]').simulate('change', { target: { value: 'notavalidemail' } });
        wrapper.find('button[type="submit"]').simulate('click', { preventDefault: jest.fn() });
    ...
    it('should show loader with attemptingRegister', () => {
        const startRegister = jest.fn();
        const wrapper = shallow(<RegisterForm startRegister={startRegister} attemptingRegister={true} />);
    ...
    it('should correctly map state to props', () => {
        ...
        expect(mapStateToProps(mockState).attemptingRegister).toEqual(mockState.auth.attemptingRegister);
        expect(mapStateToProps(mockState).errorMsg).toEqual(mockState.auth.registerErrorMsg);
    });
});
